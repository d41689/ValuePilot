...
type StockSummaryCardProps = {
  ...
  currentPriceFreshness?: string | null;
  currentPriceReason?: string | null;
  reportPriceReference?: number | null;
  reportPriceReferenceProvenanceLabel?: string | null;
  pe?: number | null;
...
export default function StockSummaryCard({
  ...
  currentPriceSource,
  currentPriceFreshness,
  currentPriceReason,
  reportPriceReference,
  reportPriceReferenceProvenanceLabel,
  pe,
  activeReportDate,
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-xl border border-border/60 bg-card/80 p-4">
          ...
          <div className="mt-2 text-2xl font-semibold">
            {currentPriceCurrency ? `${currentPriceCurrency} ` : ''}{formatNumber(price, 2)}
          </div>
          <div className="mt-2 text-xs text-muted-foreground">
            {price !== null && price !== undefined
              ? [currentPriceDate, currentPriceSource, currentPriceFreshness].filter(Boolean).join(' · ')
              : `Unavailable · ${currentPriceReason || currentPriceFreshness || 'price_missing'}`}
          </div>
          {priceProvenanceLabel ? (
            <div className="mt-2 text-xs text-muted-foreground">{priceProvenanceLabel}</div>
          ) : null}
          {reportPriceReference !== null && reportPriceReference !== undefined ? (
            <div className="mt-3 border-t border-border/60 pt-3 text-xs text-muted-foreground">
              Value Line report price reference: {formatNumber(reportPriceReference, 2)}
              {reportPriceReferenceProvenanceLabel
                ? ` · ${reportPriceReferenceProvenanceLabel}`
                : ''}
            </div>
