...
class ValueLineRatioCalculator:
    ...
    def calculate_for_stock(self, *, user_id: int, stock_id: int) -> list[MetricFact]:
        # Retire the complete prior projection before rebuilding. If a corrected
        # input makes a ratio unavailable, the old ratio must not remain current.
        self.db.execute(
            update(MetricFact)
            .where(
                MetricFact.user_id == user_id,
                MetricFact.stock_id == stock_id,
                MetricFact.metric_key.in_(sorted(OUTPUT_KEYS)),
