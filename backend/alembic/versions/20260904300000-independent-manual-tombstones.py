"""Permit each manual rationale field one independent tombstone transition.

Revision ID: 20260904300000
Revises: 20260904290000
"""
...
revision = "20260904300000"
down_revision = "20260904290000"
branch_labels = None
depends_on = None


_INDEPENDENT_TOMBSTONE_FUNCTION = r"""
...
def upgrade() -> None:
    op.execute(_INDEPENDENT_TOMBSTONE_FUNCTION)
...
def downgrade() -> None:
    ...
    if connection.execute(
        sa.text(
            ...
            "AND value_json ? 'redaction_note_content_hash'"
        )
    ).scalar_one():
        raise RuntimeError(
