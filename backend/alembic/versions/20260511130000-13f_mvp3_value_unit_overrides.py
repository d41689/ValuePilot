...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("old_parse_rule", sa.String(length=80), nullable=True),
        ...
        sa.Column("created_parse_run_id", sa.BigInteger(), nullable=True),
        sa.Column("status", sa.String(length=30), server_default="pending_reparse", nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.ForeignKeyConstraint(["created_parse_run_id"], ["parse_runs.id"], ondelete="SET NULL"),
