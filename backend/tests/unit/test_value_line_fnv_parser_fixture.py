...
FIXTURE_PDF = Path("tests/fixtures/value_line/FNV.pdf")
EXPECTED_JSON = Path("tests/fixtures/value_line/FNV_v1.expected.json")
...
def test_value_line_fnv_page_json_matches_expected_fixture():
...
def test_value_line_fnv_narrative_and_identity_are_extracted():
    actual = build_page_json()

    assert actual["identity"]["ticker"] == "FNV"
    assert actual["narrative"]["business"] is not None
    assert actual["narrative"]["analyst_commentary"] is not None
