# Quant Trading System — Framework / Architecture (v9 — ACCEPTED, Phase 1 ticketing)
...
- **1-R0 Data-sufficiency audit + power analysis (BLOCKING — added v9, master review D13).** Before any engine code: (a) quantify the actual historical depth and breadth of `metric_facts` (years covered × stocks per monthly cross-section × metric coverage; empirical status 2026-07-02: dev and prod are **empty**); (b) run a pre-registered **power analysis** — given the 1-R4 α threshold (≥2%/yr) and a realistic tracking-error assumption, compute the minimum `T × breadth` needed for `t_HAC ≥ 3` including the holdout split; (c) decide the data plan: acquire **historical Value Line archives** (VL sells historical datasets; university libraries hold decades of print/digital editions), extend the accumulation window, or conclude the hypothesis is not currently testable. **Gate: 1-R1…1-R4 do not start until the audited data meets the power requirement.** Running 1-R4 underpowered is forbidden — an inconclusive test burns the holdout for nothing.
...
- **1-R3 Cross-Sectional Factor Engine** — lightweight Python/Pandas factor evaluator (decile analysis, IC, turnover estimate, sector/size neutralization, Fama-French exposures). Offline, Docker-run; no execution rails.
- **1-R4 OOS Signal Gate + Differentiated-Moat Test (research gate — before any rails; FAIL = halt Phase 2, do not build).** Two sub-gates, both **pre-registered in the Edge Thesis** and evaluated **once** on an untouched final holdout:
  ...
  - **(b) Differentiated-moat test — controlled spanning regression (reviewer M1/M5, hard-blocking).** Regress the proprietary Value Line signal portfolio's excess returns on a **fixed, strong generic null**: FF5 + momentum + a **same-universe** generic value/quality/momentum composite built from commodity fundamentals — identical universe, rebalance cadence, sector/size neutralization, and cost/tax model (no strawman null). The moat exists **only if** the intercept α is:
  ...
  - **Kill rule (replaces "re-evaluate"):** if either sub-gate fails, **Phase 2 rails are not built.** The project halts to rethink the signal — it does **not** tune parameters against the holdout. (This research gate is distinct from and precedes the pre-live **Go/No-Go Profitability Gate** in [§13](#13-validation--overfitting-discipline).)
  - **Post-kill fallback (pre-committed, v9):** a 1-R4 failure kills the *moat claim*, not the capital plan. The pre-registered fallbacks are: **(a)** implement **generic factors without moat claims** as a personal, tax-located (IRA-first), discipline-enforced allocation — for personal capital the bar is beating one's own passive after-tax alternative, not beating institutions; and/or **(b)** pivot the data-edge hypothesis to the **13F aggregation signals** already ingested by ValuePilot (EDGAR pipeline exists) and re-enter at 1-R0 with a new pre-registered Edge Thesis. Base-rate honesty: Value Line rank/opinion effects are among the most-arbitraged documented anomalies — **failure is the likely outcome and is a legitimate, informative result.**
