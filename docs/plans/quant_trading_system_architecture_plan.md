# Quant Trading System — Framework / Architecture (v11 — 1-R0A empirical gate)

**Status:** ACCEPTED design; implementation is stopped at the first empirical gate. `1-R0A` shipped on 2026-07-21 with an explicit **NO_GO**; `1-R1…1-R4` remain closed.
**Review state:** v2–v10 design reviews closed; D13 added the blocking 1-R0 audit. D15 records the implemented 2026-07-21 audit and supersedes the older “empty/testable now” empirical assumptions with measured development data. Adversarial verdict: audit implementation APPROVED, Phase 1 continuation REJECTED.
...
| **D15** | **2026-07-21 1-R0A empirical audit → NO_GO.** The development DB has 3 user-scoped Value Line reports (768 parsed facts, 3 stocks), only 2 non-consecutive report weeks, and 13F availability of 3.526 years despite 18 quarter labels. Correct power planning adds an explicit 80% target power and separates time from breadth. At α=2%/yr, TE=4%, `t>=3`, 30% final holdout, H1 needs 709 holdout months / ~197 total years; H3 at 6% TE needs 532 holdout quarters / ~443.5 total years. The former “H1/H3 testable now” wording is superseded. Also, historical H3 must select the filing/amendment version observable at T; today's active filing is current-product authority only. | A statistical gate must be satisfiable by actual PIT data. Buying an arbitrary commercial feed or counting more stocks cannot repair inadequate time depth. Stop before 1-R1 and version the statistical/data policy before touching a holdout. |
...
- **1-R0 Data-sufficiency audit + power analysis (BLOCKING — implemented 2026-07-21, result NO_GO).** The repeatable report is `docs/audits/quant/2026-07-21_1-r0-data-sufficiency.md`. It (a) separates strict publication-vintage depth from restated fiscal periods; (b) computes required return-history time with explicit 80% target power and a 30% holdout sensitivity while enforcing cross-sectional breadth separately; (c) audits Value Line authorization/archive continuity, the survivorship-free backbone, prices, and versioned 13F availability. Current result: **H1/H2/H3 NO_GO; 1-R1…1-R4 closed.** Next permitted work is a versioned statistical-design/data-procurement decision plus licensed-source evidence (`1-R0B`), not engine code. Value Line automated acquisition remains blocked by `coverage-source-policy.md` until operator authorization exists.
- **1-R1 Database PIT Read Engine** — Implement historical PIT read contract (§9 rules 1–4, 6) for parsed facts **and the §9/architecture H3 filing-amendment rule**: choose the 13F version observable at T; never reuse today's active-filing selector for historical reads. **Closed until 1-R0 becomes GO.**
...
- **1-R4 OOS Signal Gate + Moat Hypothesis Set (research gate — before any rails; FAIL = halt Phase 2, do not build).** Sub-gate (a) plus a **hypothesis set (v10 D14)** replacing the single moat test — all pre-registered in the Edge Thesis, evaluated **once** on an untouched final holdout:
  ...
  - **(b) Moat hypothesis set — controlled spanning regressions (reviewer M1/M5, hard-blocking).** Each hypothesis regresses its signal portfolio's excess returns on the **same fixed, strong generic null**: FF5 + momentum + a **same-universe** generic value/quality/momentum composite built from commodity fundamentals — identical universe, rebalance cadence, sector/size neutralization, and cost/tax model (no strawman null):
    - **H1 — VL-proxy composite (designed; NOT currently powered):** once an independently verified survivorship-free commodity backbone passes 1-R0, does the VL-style *combination* (1-R3 library) add α beyond the standard null? Tests the construction, not the data source.
    ...
    - **H3 — 13F aggregation signals (in-house candidate; NOT currently powered):** after 1-R0 and the historical filing/amendment PIT selector pass, do Oracle's-Lens-style institutional-holdings signals (conviction clones, crowding, 13F momentum) add α beyond the null? This is an in-house asset, but 3.526 years of actual filing availability cannot satisfy policy v1 power.
...
*End of v11 (1-R0A empirical amendment 2026-07-21). Implementation is stopped at `NO_GO`; no later Phase 1 or Phase 2 task is authorized until a versioned 1-R0 policy passes.*
