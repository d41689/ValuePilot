...
CREATE TABLE raw_source_documents (
    ...
    document_type       VARCHAR(40) NOT NULL,         -- submissions_json / form_idx / filing_directory / primary_doc_xml / infotable_xml / dataroma_managers_html / dataroma_holdings_html
    cik                 VARCHAR(10),
...
2. 构建文件目录 URL，拉取 primary-doc.xml 与 infotable.xml
...
- [ ] manager name → CIK 候选匹配器（输出匹配分数，低分进入人工 review）
- [ ] `form.idx` 解析器（季度索引）
