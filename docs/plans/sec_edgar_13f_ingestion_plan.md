...
-- 原始抓取文档（可重放、可审计）
CREATE TABLE raw_edgar_documents (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    source_type         VARCHAR(30) NOT NULL,         -- submissions_json / filing_index / filing_directory / primary_doc_xml / infotable_xml
    cik                 VARCHAR(10),
...
CREATE TABLE filings_13f (
    ...
    raw_primary_doc_id              BIGINT REFERENCES raw_edgar_documents(id),
    raw_infotable_doc_id            BIGINT REFERENCES raw_edgar_documents(id),
...
2. **normalized layer**：将原始文档解析入 `filings_13f`、`holdings_13f`、`cusip_ticker_map`
...
1. 从 full-index 季度索引获取所有 13F-HR / 13F-HR/A 申报元数据
   GET https://www.sec.gov/Archives/edgar/full-index/{year}/QTR{n}/form.idx
   → 先将原始 index 文件写入 raw_edgar_documents
...
3. 先将原始 XML 写入 raw_edgar_documents
...
- [ ] 数据库 migration（`institution_managers`、`raw_edgar_documents`、`filings_13f`、`holdings_13f`、`cusip_ticker_map`）
...
| infotable.xml 格式变化 | 容错 XML 解析，记录失败 raw 文档，可重放解析 |
