...
CREATE TABLE raw_source_documents (
    ...
    document_type       VARCHAR(40) NOT NULL,         -- 取值必须受代码枚举约束：submissions_json / form_idx / primary_doc_xml / infotable_xml / dataroma_managers_html / dataroma_holdings_html（filing_directory 按需再加）
    cik                 VARCHAR(10),
...
-- 当前 MVP 阶段不保留同一 URL 的多次抓取历史；重抓默认视为幂等覆盖或跳过。
...
CREATE TABLE filings_13f (
    ...
    reported_total_value_thousands  BIGINT,                        -- 封面页汇总值（千美元）
    computed_total_value_thousands  BIGINT,                        -- infotable 汇总值（千美元）
...
3. 先将原始 XML 写入 raw_source_documents（source_system='edgar', document_type='primary_doc_xml' / 'infotable_xml'）
