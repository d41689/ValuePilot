...
CREATE TABLE raw_source_documents (
    ...
    document_type       VARCHAR(40) NOT NULL,         -- submissions_json / form_idx / primary_doc_xml / infotable_xml / dataroma_managers_html / dataroma_holdings_html（filing_directory 按需再加）
    cik                 VARCHAR(10),
    ...
    parsed_at           TIMESTAMPTZ,                  -- 最近一次解析尝试完成时间；成功/失败由 parse_status 区分
    error_message       TEXT
);
...
`document_type` 取值必须受代码枚举约束，不允许自由文本：`submissions_json` / `form_idx` / `primary_doc_xml` / `infotable_xml` / `dataroma_managers_html` / `dataroma_holdings_html`（`filing_directory` 按需扩充）。
...
CREATE TABLE filings_13f (
    ...
    accession_no                    VARCHAR(20) UNIQUE NOT NULL,   -- 统一保存带横杠格式（0001234567-24-000001）；构造 EDGAR 文件目录 URL 时再转换为去横杠格式
    ...
    reported_total_value_thousands  BIGINT,                        -- 来自 13F 封面/summary 信息，不从 infotable 反推（千美元）
    ...
    UNIQUE (manager_id, period_of_report, accession_no)
);
...
- 若 `reported_total_value_thousands` 与 `computed_total_value_thousands` 不相等，则记录告警
- MVP 阶段优先采用严格告警；后续可根据实测数据放宽阈值
...
7. 对每个新 CUSIP，触发 cusip_ticker_map 填充（先查 Dataroma，再查 EDGAR）
```
...
- 同一期内保留所有版本
- serving 层默认只暴露 `is_latest_for_period = true` 的 canonical snapshot
- 如需审计或对比，可按 accession_no 查看历史版本
```
