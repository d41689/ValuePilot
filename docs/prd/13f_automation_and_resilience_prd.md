...
`no_index_expected_dates` 维护要求：周末和 US OPM federal holidays 可由 calendar provider 自动生成；EDGAR 特别关闭日由 ops 根据 SEC/EDGAR 官方公告手动加入。Admin UI 必须支持新增/停用这些日期且不需要代码部署，避免临时关闭导致 daily sync 误报失败。
...
`holding_row_fingerprint` 必须只锚定原始 holding 行内容，不包含 `parse_run_id` 这类运行时 ID；这样同一 accession 在不同 parse run 中的相同行可用于跨 run 对比。唯一约束仍为 `UNIQUE (parse_run_id, holding_row_fingerprint)`，用于保证同一 parse run 内不重复写入。
...
**Original filing 冲突处理：**

若同一 manager + report period 出现多个非 amendment 原始 filing（例如 13F-HR 与 13F-NT 同期并存，或 filer 误报后重新提交原始 accession），不得让 `uq_active_filing_per_manager_period` 直接触发数据库错误。系统按 `accepted_at` 最新者自动设为 `is_active_for_manager_period=true`，其他原始 filing 保留审计记录并标记 `needs_review`；若 `accepted_at` 完全相同或排序不确定，则不自动切 active，设置 `amendments_pending + amendment_sort_warning=true` 等待 admin 确认。
...
coverage_type                         -- normal | combination_partial | notice_reported_elsewhere
                                      -- 主覆盖语义（coverage structure）：normal=普通 HR，combination_partial=Combination Report，notice_reported_elsewhere=13F-NT
                                      -- confidential treatment 是正交的数据完整性信号，走独立 has_confidential_treatment bool；不在此枚举
                                      -- 一个 filing 可以同时是 combination_partial 且 has_confidential_treatment=true，两者互不排斥
...
report_quarter
quarter_end_date
official_filing_deadline              -- quarter_end + 45 days，调整到下一个 SEC/EDGAR operational business day（SEC federal holiday calendar + EDGAR 特别关闭日）
is_amendment
...
- `coverage_gap` 不是 `filings_13f.coverage_type` 的可存储值；它表示 manager-quarter 无 filing row，应由 readiness/query layer 从缺失 active filing 推导，并暴露在 `data_gap_quarters`。
...
- **`filings_13f.parse_status` 枚举：** `pending | succeeded | failed | partial_success | needs_review`。`pending` = 已发现未处理；`succeeded` = parse_run 成功完成；`failed` = 最近一次 parse 失败可重试；`partial_success` = 部分 holdings 解析失败；`needs_review` = 需人工确认（缺失 period、格式异常等）。**`parse_runs.status` 枚举（独立）：** `running | succeeded | failed | abandoned`。`abandoned` = 进程崩溃后 watchdog 标记，可重试；与 `filings_13f.parse_status` 独立维护，不直接同步（parse_runs.status 追踪单次执行，filings_13f.parse_status 聚合 active 结论）。
...
cusip_mapping_status                  -- holding-level CUSIP enrichment result: linked | invalid_cusip | unresolved | pending_mapping | needs_review
                                      ...
                                      -- linked 当且仅当 stock_id IS NOT NULL；质量指标以 stock_id IS NOT NULL 为权威 linked signal
portfolio_weight_pct                  -- (value_usd / filing.total_13f_common_value_usd) * 100
                                      ...
                                      -- 若 total_13f_common_value_usd 为 0 或 NULL，则必须为 null，避免除零或误导性权重
...
1. **优先**：从 EDGAR XML root / namespace / schemaLocation / form_spec_version 确定单位声明；parser 必须显式识别 `accepted_at >= 2023-01-03` 起 amended Form 13F 的 nearest-dollar 口径（而非 nearest-thousand）。
...
注：`2023-01-03` 指 filing submission / `accepted_at` 日期，不是 `period_of_report`。例如 Q4 2022（`period_of_report=2022-12-31`）filing 通常在 2023 年 1–2 月提交，若使用 amended Form 13F XML/schema，应按 dollars 口径解析；fallback heuristic 不得仅按 report quarter 判断单位。
...
**13F-NT 的 holdings 查询契约（query contract）：**

Holdings endpoints（`GET /managers/{id}/holdings`、`GET /stocks/{id}/holders` 等）只 join `form_type IN (13F-HR, 13F-HR/A)` 且 `is_active_for_manager_period=true` 的 filing 的 current parse_run，再读取 `holdings_13f`。

13F-NT 的 active filing（`coverage_type=notice_reported_elsewhere`）：
- **不进入 holdings 查询路径**。
- 不创建 parse_run，不创建 holdings_13f 行。
- 只参与 coverage / readiness / caveat 逻辑（expected filer exclusion、streak 状态、UI 13F-NT caveat 文案）。

违反此契约会导致 NT manager 的 holdings 查询返回空结果，被前端误读为"无持仓"——这正是 PRD 明令禁止的错误语义（见 §2.2, §16）。
...
这些 heuristic signal 在 UI/API 中必须明确标记为 "Possible Split/Merger" 或同等级别的不确定提示；不得用于自动调整 `value_usd`、`ssh_prnamt` 或历史 holdings，也不得把 possible corporate action 展示为确定事实。
...
- **MVP 方案（推荐）：** 写入 cusip_ticker_map 前，在同一事务内取基于 canonical CUSIP 的 64-bit advisory lock key（例如 `pg_advisory_xact_lock(hashtextextended(cusip, 0))`，或 SHA256 派生的两个 32-bit key），再做 overlap 检查，再 INSERT。事务结束时锁自动释放；同一 CUSIP 的并发写入序列化执行。不要使用 `hashtext(cusip)` 作为唯一锁 key，因为 32-bit hash collision 会让无关 CUSIP 不必要地互相阻塞。
...
| `has_confidential_treatment=true` | 不中断，但该季度 holdings 可能不完整 | `has_confidential_treatment=true` 驱动 caveat 和 readiness cap；不修改 `coverage_type`（confidential 与 coverage structure 正交）|
...
-- Admin: No-index calendar 维护（MVP 1A）
-- no_index_expected_dates 表最低字段: date (PK), reason (weekend|federal_holiday|edgar_special_closure|other), holiday_name (text, optional), source (auto_generated|admin_manual), note (text, optional), active (bool), created_by, created_at
-- 周末/federal holiday 由系统自动生成 (source=auto_generated)；EDGAR 特别关闭日由 ops 手工添加 (source=admin_manual)
GET  /api/v1/admin/13f/no-index-dates              -- 列出所有日期（支持 ?year= 过滤）
POST /api/v1/admin/13f/no-index-dates              -- 新增特别关闭日（source=admin_manual；不允许手工创建 weekend/federal_holiday 行）
PATCH /api/v1/admin/13f/no-index-dates/{date}      -- 更新 note 或停用（active=false）；不支持物理删除
...
- API / 前端 label 中 `portfolio_weight_pct` 统一显示为 **"13F common weight"**；当 `total_13f_common_value_usd` 为 0 或 NULL 时显示 unavailable，不显示 0%。

**13F-NT Manager：** 展示为 *"This manager filed a 13F Notice; its 13(f) holdings are reported by other manager(s)."* 不得显示为数据缺失、空持仓、或 "No holdings"（因 holdings 客观存在，只是在其他 manager 的 filing 中）。若已解析 `other_managers_reporting`，补充展示：*"Holdings for this manager are reported by [Other Manager Name]. [Other Manager] is currently tracked / not tracked by ValuePilot."*
...
**MVP 1B 前置 Spike（必须在 parser 实现开始前完成，关闭 §20 对应 open question）：**
收集 2022 及以前、2023 及以后的 EDGAR 13F XML 样本（各至少 2 个真实 filing），验证 XML namespace / schemaLocation / form_spec_version 到 `schema_thousands / schema_dollars` 的完整映射表。以 SEC 官方文档（Form 13F Instructions、EDGAR FAQ Q36 或 EDGAR XML schema 定义）确认规则。Spike 输出物：mapping 规则文档 + 对应 fixture 文件，纳入 parser test suite。**Spike 未完成前不得进入 parser 正式实现。**
...
- If `total_13f_common_value_usd` is 0 or NULL, `portfolio_weight_pct` is NULL and UI displays unavailable rather than 0%.
...
- Given a manager submits multiple non-amendment original filings for the same quarter, the latest `accepted_at` filing becomes active and older originals are retained as `needs_review`; if ordering is ambiguous, activation requires admin review.
...
| 历史回补默认起始季度 | `DEFAULT_BACKFILL_START_QUARTER=2023-Q1`，环境变量可覆盖；该默认值与 2023-01-03 Form 13F value unit 切换相关。注意 Q4 2022 filings 通常在 2023-01-03 之后提交并可能使用 dollars，早于 2023-Q1 的 backfill 也必须按 `accepted_at` + XML schema 启用 thousands/dollars 双口径 fixtures |
...
| CUSIP overlap 并发控制 | MVP: 使用 canonical CUSIP 派生的 64-bit advisory lock key 在同一事务内加锁，overlap 检查后再写入；长期: daterange + EXCLUDE USING gist exclusion constraint |
