...
f6cf44e0 的完整后端结果为 2 failed / 2,780 passed（1,337.67 秒），两项独立重跑仍失败。
DCF 跨纽约日测试固定读取 2026-09-10 01:30 UTC，此时测试新建的事实/authority 已晚于
cutoff，正确被 PIT 排除；Value Line 第 121 天测试用容器 date.today()，产品按数据库
时刻的纽约业务日评估，UTC 午夜附近相差一天。不是 S1 产品规则需要放宽的证据。
为完成可信全套验收，必要最小修改仅在两项测试：读取时刻置于测试数据创建之后，保持
同一 UTC 日跨纽约日的 DCF 负例；覆盖率测试明确参数化 UTC/纽约日期不同及相同的时段。
不回填数据库时间、不跳过测试、不改变预期拒绝/过期行为。修改前两个测试均 red。
内部浏览器已实际显示 case 1 的 AAPL 财务及授权证据链接，不再停在 loading；原 UI 的
60 行前缀仍在，仅前两个指标可见，这仍是 S2 必须解决的展示缺口。
