...
## 2026-09-22 consolidation integration review

The user authorized consolidating and merging the reviewed branches, then explicitly included the populated-upgrade prerequisite repair (#151). Historical stacked-base descriptions above remain execution records. Final delivery will target main after S2 #149; no business-data replay or raw artifact upload is part of this pass.

Independent read-only review of local combination `63ac6860` (MCO `03550aa7` + S2 `d9d337bd`, including repaired main `234876be`) is PASS. S2 unavailable-evidence redaction, cached-head conflict and pending-save draft protections, and the populated-upgrade repair/test match S2 exactly; all four MCO migrations match the previously reviewed MCO head exactly. Focused Docker tests passed: 25 frontend checks and 30 backend checks (2 existing warnings) on a new isolated synthetic PostgreSQL instance, including the populated 140000-to-current-head path and migration/parser authority guards. Temporary resources were removed; retained/shared/production data was untouched.

This evidence does not replace the final retargeted SHA's complete canonical CI. Retarget to main, incorporate the completed S2 merge, push and verify that final combination before merging; inspect the existing main CI and automatic production deployment afterward. Existing scope exclusions and backlog links above remain in force.

### Combined full-gate fixture mismatch follow-up

Final combined CI35787456007 stopped at backend tests: 3 failed / 2,949 passed. All failures are the old-parser replay evidence assertions for v2.9/v2.10/v2.11, expecting filing proof from the default fixture. S2 explicitly treats that fixture's missing retained labels as `evidence_text_unavailable` and redacts all partial proof. Investigate and reproduce this distinction before editing: prove old-parser evidence remains readable with complete retained statement text, and prove the original incomplete fixture stays typed unavailable/redacted after exact replay. Scope is a faithful test-fixture/contract repair if confirmed; do not loosen evidence authority, fabricate retained labels in production, remove replay identity/count assertions or bypass unavailable redaction. Rerun focused combined tests, independent review, then all canonical CI commands on the repaired head.

Independent diagnosis confirmed all three old versions exactly replay their original facts; default compact XML lacks retained row labels and is correctly unavailable, while the existing complete synthetic client is readable for all three. The test-only repair keeps original run/fact/publication identity and row-count checks, explicitly proves readable complete evidence, and adds three negative cases proving missing labels stay unavailable with numeric/exact value, inputs, filings and locator redacted. No production file changed. Local RED reproduced all three prior failures; GREEN is 11/11 in the owned test file plus 24/24 adjacent evidence/API cases. A separate read-only reviewer reran all three files: 35/35 passed, 2 existing warnings, PASS with no scoped finding. Test resources contain synthetic data only and are removed after review. A fresh complete canonical CI remains the merge gate.
