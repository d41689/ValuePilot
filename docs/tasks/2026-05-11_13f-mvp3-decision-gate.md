...
Implementation choice:
- MVP3-05 should decide whether to ship quarter reparse first, manager reparse first, or both behind the same admin workflow.
...
7. `MVP3-07 Historical Backfill Job Contract and Preview`

## Approval Checklist

- [ ] D1 historical backfill default range approved as 2023-Q1 onward; pre-2023-Q1 limited to explicitly approved curated dry-run / validation mode.
- [ ] D2 human owner explicitly approves scoping out PRD §17 `Dataroma CUSIP 来源` from MVP 3 as a PRD scope change, and approves legacy / non-authoritative wrapper cleanup or renaming.
- [ ] D3 controlled reparse contract and before/after impact summary requirement approved before batch reparse.
- [ ] D4 manual-confirmation-only corporate action evidence policy approved.
- [ ] D5 filing-level `value_unit_override` storage/audit model approved; filing-scoped per PRD §20, uses effective filing pointer plus separate audit table, and does not modify or replace the existing manager-level `value_unit_override=infer` column.
- [ ] D6 persisted quality reports approved as validation source of truth, with alerts only as notification surfaces.
